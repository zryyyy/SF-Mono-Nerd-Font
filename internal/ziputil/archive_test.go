...
func TestCreateZipWithDirectoryEntries(t *testing.T) {
	temp := t.TempDir()
	a := filepath.Join(temp, "A.ttf")
	b := filepath.Join(temp, "B.otf")
	if err := os.WriteFile(a, []byte("a"), 0o644); err != nil {
	...
	zipPath := filepath.Join(temp, "out", "all_fonts.zip")
	if err := Create(zipPath, []Entry{
		{Source: a, Name: "GroupA/A.ttf"},
		{Source: b, Name: "GroupB/B.otf"},
	}); err != nil {
	...
	sort.Strings(names)
	if want := []string{"GroupA/A.ttf", "GroupB/B.otf"}; !slices.Equal(names, want) {
		t.Fatalf("unexpected zip entries: got %v want %v", names, want)
	}
}
